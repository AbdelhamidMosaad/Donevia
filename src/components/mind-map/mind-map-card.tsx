...
export function MindMapCard({ map, onDelete, size = 'large' }: MindMapCardProps) {
  const [editingName, setEditingName] = useState(map.name);
  ...
  const handleRename = async () => {
    if (!user || !editingName.trim() || editingName === map.name) {
      setIsEditing(false);
      setEditingName(map.name);
      return;
    }

    const mapRef = doc(db, 'users', user.uid, 'mindMaps', map.id);
    try {
  ...
  return (
    <a href={`/mind-map/${map.id}`} onClick={handleCardClick} className="group block h-full">
      <Card className="relative h-full overflow-hidden rounded-2xl bg-card/60 backdrop-blur-sm border-white/20 shadow-lg transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-xl cursor-pointer">
        <div className={cn("p-6 flex flex-col items-center text-center h-full justify-center", size === 'medium' && 'p-4', size === 'small' && 'p-3')}>
            ...
            {isEditing ? (
              ...
              <h3 className={cn("font-bold font-headline text-foreground", size === 'large' && 'text-lg', size === 'medium' && 'text-base', size === 'small' && 'text-sm')}>{map.name}</h3>
            )}
            {size !== 'small' && <p className="text-xs text-muted-foreground mt-1">
                {map.updatedAt && typeof map.updatedAt.toDate === 'function'
                ? `Updated ${moment(map.updatedAt.toDate()).fromNow()}`
                : 'Just now'}
            </p>}
        </div>
        <DropdownMenu>
            ...
            <AlertDialog>
                ...
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                    This will permanently delete the "{map.name}"
