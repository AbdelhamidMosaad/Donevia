...
function AdvancedMindMap() {
    ...
    // --- History Management ---
    const pushHistory = (currentNodes: Record<string, Node>, currentConnections: Connection[]) => {
    ...
    const renderNode = (node: Node) => {
        ...
        const width = Math.max(120, (node.text?.length || 0) * (node.fontSize / 1.5)) + 20;
    ...
    <div className="w-full h-full bg-gray-900 flex flex-col overflow-hidden" ref={containerRef}>
        <div className="bg-gray-800 p-2 flex items-center gap-2 border-b border-gray-700 flex-wrap">
            <Button variant="ghost" size="icon" onClick={() => router.push('/mind-map')}><ArrowLeft className="text-white"/></Button>
            <Input className="text-xl bg-transparent border-0 text-white w-auto" value={boardName} onChange={(e) => setBoardName(e.target.value)} />
            ...
            <Button asChild variant="outline"><label htmlFor="import-file" className="cursor-pointer flex items-center gap-2"><Upload size={18} /> Import</label></Button>
        ...
        {selectedNode && (
            <div className="absolute top-16 left-1/2 -translate-x-1/2 z-10 bg-gray-800 p-1 flex items-center gap-1 border border-gray-700 rounded-lg">
                ...
                <Button variant="ghost" size="icon" onClick={() => changeFontSize(selectedNode, -2)}><Type size={18} />-</Button>
                <Button variant="ghost" size="icon" onClick={() => { const node = nodes[selectedNode]; const idx = shapes.indexOf(node.shape); changeShape(selectedNode, shapes[(idx + 1) % shapes.length]); }}><Circle size={18} /></Button>
            </div>
        )}
        
        {editingNode && (
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-800 p-6 rounded-lg shadow-2xl z-20 border border-gray-700">
                <Input
                    ref={editInputRef}
                    type="text"
                    value={editText}
        ...
        >
            <svg ref={canvasRef} width="100%" height="100%">
                <g style={{ transform: `translate(${offset.x}px, ${offset.y}px) scale(${zoom})`, transformOrigin: '0 0' }}>
