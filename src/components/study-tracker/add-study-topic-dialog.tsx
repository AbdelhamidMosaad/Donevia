...
import type { StudyTopic, StudyTopicResource, StudyDifficulty } from '@/lib/types';
import { addStudyTopic, updateStudyTopic } from '@/lib/study-tracker';
import { Textarea } from '../ui/textarea';
import { PlusCircle, Trash2 } from 'lucide-react';
import { v4 as uuidv4 } from 'uuid';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '../ui/select';

interface AddStudyTopicDialogProps {
  goalId: string;
  chapterId: string;
...
export function AddStudyTopicDialog({
  goalId,
  chapterId,
  topic,
  open,
  ...
  const [resources, setResources] = useState<StudyTopicResource[]>([]);
  ...
  const handleSave = async () => {
    ...
    console.log('Attempting to save topic:', title);
    setIsSaving(true);
    const topicData = {
        goalId,
        chapterId,
        ...
        order: topic?.order ?? topicsCount,
    };

    try {
      if (isEditMode && topic) {
        await updateStudyTopic(user.uid, topic.id, topicData);
        toast({ title: 'Topic Updated' });
      } else {
        await addStudyTopic(user.uid, topicData);
        toast({ title: 'Topic Added' });
      }
      onOpenChange?.(false);
    } catch (e) {
      console.error("Error saving topic: ", e);
      toast({ variant: 'destructive', title: 'Error', description: 'Failed to save topic.' });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl" onKeyDown={handleDialogKeyDown}>
        <DialogHeader>
          ...
          <DialogDescription>{isEditMode ? 'Update this topic.' : 'Add a new topic to this chapter.'}</DialogDescription>
        </DialogHeader>
